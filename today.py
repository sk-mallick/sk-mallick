...
def add_archive():
    ...
    try:
        commit_str = old_data[-1].split()[4][:-1]  # remove trailing period from e.g. '543.'
        added_commits += int(commit_str)
    except (IndexError, ValueError):
        pass  # no archived commit total found
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    root = tree.getroot()
    justify_format(root, 'uptime_data', age_data, 23)
...
if __name__ == '__main__':
    """
    Subham Kumar Mallick (sk-mallick)
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2004, 8, 8))
